...
def fetch_data(url, header):
    ...
    if res.status_code in (200, 201):
        # jq = PyQuery(res.text)
        file = open('xiechen2.txt', 'w', encoding='utf-8')
        file.write(res.text)
        file.close()
        print('success')
        # data: dict = {}
...
def parse_html(html: str = ''):
    if not html:
        with open('test1.txt', 'r', encoding='utf-8') as f:
            html = f.read()
    jq = PyQuery(html)
    company_name, phone, status_quo, legal_representative, registered_capital, register_address, email, date_of_establishment = ('',) * 8
    search_result = jq("#search-result")
    if search_result:
        ul_tag = jq("ul.pagination.pagination-md").children()
        if ul_tag:
            get_last_element = ul_tag[-1]
            get_last_element_val = get_last_element
            print(get_last_element_val)
        else:
            tr_tag = search_result.children()

    else:
        alert = jq("div.noresult.no-search").text()
        print(alert)
